...
def check_positives(path,output):
    ...
    #get groups of 100
    photo_list=chunk_iter(photos_run,100)
    
    for photos in photo_list:
        image_array=[]
        
        for x in photos:
            image_array.append(cv2.imread(x))
            
        pred=TensorflowPredict(read_from="numpy",sess=sess,image_array=image_array,label_lines=["Positive","Negative"])
        
        for image,[label,score] in enumerate(pred):
            if label == "Negative":
                font = cv2.FONT_HERSHEY_COMPLEX         
                cv2.putText(image_array[image],str(pred),(10,20), font, 0.5,(255,255,255),1)            
                cv2.imshow("Annotation", image_array[image])
                cv2.waitKey(1)            
                counter+=1                
...
if __name__ == "__main__":
    ...
    tf.saved_model.loader.load(sess,[tf.saved_model.tag_constants.SERVING], "/Users/ben/Dropbox/GoogleCloud/DeepMeerkat_20170924_105144/model/")    
    print("Model loaded")
    
    #Interactive
    interactive("/Users/ben/Dropbox/HummingbirdProject/Data/Yanacocha/foundframes/201707/YANA3207/170717AB/*.jpg")
